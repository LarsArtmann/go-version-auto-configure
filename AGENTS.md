...
- **license-check (go-licenses) fails intermittently on std packages without module info** ("Package net/mail does not have module info. Non go modules projects are no longer supported", google/go-licenses#128; also errors on sync/atomic, maps, log/slog, encoding/xml). Surfaced 2026-09-26 on three consecutive runs, then green again (~10% historical fail rate); `net/mail` entered the dep graph via a cmdguard/v4 bump. Watch for recurrence; the durable fix is upstream (go-licenses) or a BuildFlow-side ignore.
