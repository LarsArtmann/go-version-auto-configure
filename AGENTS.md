...
- **cqrs-lint reports 2 info findings here, but this repo imports no go-cqrs-lite.** Re-verified 2026-09-22 (post-cmdguard migration): still zero cqrs matches in go.mod and go.sum. Suppressed via `skip_steps: [cqrs-lint]` in .buildflow.yml; un-skip if the linter learns to skip non-consumers.
