...
The supply-side campaign has now shipped (2026-09-22): go-atomic-write v0.6.0, go-error-family 7-tag v0.10.2 family, go-output v0.38.2 (17 modules), go-branded-id v0.6.0, linter-autoconfigure-sdk v0.3.0, go-finding v1.13.0, and the three sibling autoconfigurers (v0.8.2 / v0.6.4 / v0.2.1). Floors settle at minor form except the accepted poisoners and any new ones — the single source of truth for active and resolved poisoners is [docs/POISONERS.md](docs/POISONERS.md) (x/text `1.26.0`, std json/v2, and the 2026-09-25 go-health v0.4.0→v0.4.1 incident all live there).
...
- **license-check (go-licenses) fails on std packages without module info** ("Package net/mail does not have module info. Non go modules projects are no longer supported", google/go-licenses#128). Surfaced 2026-09-26 when cmdguard/v4 (auto-committed bump) pulled `net/mail` into the dep graph; go-licenses also errors on sync/atomic, maps, log/slog, encoding/xml. Pre-existing on master, unrelated to any single repo change; needs a go-licenses fix or a BuildFlow-side ignore. Full-pipeline gate red on this step until then.
