...
- **Dogfooding caveat:** the auto-commit daemon (or a `go get`-style pass) re-raises this repo's `go` directive to `go 1.26.7` between runs — every published dependency (linter-autoconfigure-sdk v0.2.0, go-finding v1.10.0/v1.12.0, go-atomic-write v0.5.x) carries that patch floor. Plain `go mod tidy` does NOT re-raise it (verified 2026-09-18: strip → tidy → stays `go 1.26`, build green), but daemon-driven get-style passes do. Expected steady state: `go.mod` drifts to 1.26.7, `/tmp/gvac fix .` re-strips it tidily. Permanent resolution = T1/T4 in TODO_LIST.md. Do NOT "fix" go.mod to match the structure-linter's "1.27.1 available" suggestion — that direction is the accidental 1.27 wave (see Floor-poisoning section).
...
- `pkg/fix` — applies `Issue.Fix` entries via `go mod edit` / `go work edit` (never sed). Every fix is verified by re-parsing the file; a fix counts as applied only when the directive actually changed. `Options.DryRun` holds fixes back.
