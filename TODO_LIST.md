...
Short- and mid-term actionable work. Ordered by impact (Pareto). Harvested from `docs/status/2026-09-18_15-13_fleet-versioning-unification.md` (section f) on 2026-09-18.
...
## T7 — Fleet-sweep enablers (tool work) — PLANNED

- [ ] Parallel repo execution + skip-if-clean fast path (a 152-repo fix sweep with per-module tidy is a multi-hour single-threaded job)
- [ ] JSON output for CI/machines
- [ ] `who-forces` command: per-repo dependency-floor matrix naming which dependency forces each directive

## T9 — Parser coverage — PLANNED

- [ ] go.work / go.mod `toolchain` directive handling (walked past today, not modeled)
- [ ] flake.lock effective Go revision parsing (flake.nix only today)
...
## T10 — Repo hygiene — PLANNED

- [ ] Full-mode BuildFlow run (race + coverage) on this repo
- [ ] .editorconfig + .gitattributes
- [ ] dependabot.yml: `open-pull-requests-limit` per the auto-fixer suggestion
